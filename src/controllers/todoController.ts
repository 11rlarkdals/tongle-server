...
const updateTodo = async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.status(200).json({ message: "Upate todo successfully", todo });
  } catch (error) {
    res.status(500);
  }
};
...
const updateTodoIsDone = async (req: Request, res: Response) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Update isDone of todo successfully", todo });
  } catch (error) {
    res.status(500).json({ message: "Error creating user", error });
  }
};
...
export { getTodos, updateTodo, createTodo, updateTodoIsDone, removeTodo };
